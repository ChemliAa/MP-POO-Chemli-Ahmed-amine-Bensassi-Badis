...
#include <algorithm>
 BourseMultiMap::BourseMultiMap(const Date& date,string path):Bourse(date)
{    
 ...
 vector<PrixJournalier>BourseMultiMap::getPrixJournaliersParDate(const Date& date)const {
 ...
 vector<PrixJournalier>BourseMultiMap::getPrixJournaliersParDateEtPrix(const Date& date,double prix)const {
    vector<PrixJournalier> resultPrixJournalier;
    auto range=historique.equal_range(date);
    for(auto it=range.first;it!=range.second;it++){
...
double BourseMultiMap:: getPrixParDateEtAction(const Date& date,string action)const{
 ...
 double BourseMultiMap:: dernierPrixDuneAction(const Date& date,string action)const{
...
vector<string> BourseMultiMap:: getActionsDisponiblesParDate(const Date& date) {
